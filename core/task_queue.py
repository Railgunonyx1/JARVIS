...
class TaskQueue:
    ...
    def _cleanup_finished(self) -> None:
        """Remove finished tasks from _tasks dict if over max_history limit."""
        with self._lock:
            # Get all finished task IDs
            finished = [
                tid for tid, t in self._tasks.items()
                if t.status in (TaskStatus.COMPLETED, TaskStatus.FAILED, TaskStatus.CANCELLED)
            ]
            # If over limit, remove oldest finished tasks
            if len(finished) > self._max_history:
                # Sort by created_at and remove oldest
                finished.sort(key=lambda tid: self._tasks[tid].created_at)
    ...
    def _run_task(self, task: Task) -> None:
        logger.info("Running: [%s] %s", task.task_id, task.goal[:60])
        try:
