...
home automation, whatever) by adding it to config/mcp_servers.json, and
JARVIS can call its tools without you writing a dedicated integration
for each service. This does NOT replace plugin_loader.py's existing
pattern — use a hand-written plugin when you want tight control over one
specific action; use this when you want to plug in an existing MCP server
fast.
